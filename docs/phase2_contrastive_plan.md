...
- The first milestone is a core SSL result, not the full paper-style suite: implement contrastive pretraining, rerun a canonical fused supervised baseline, and run paired plus single-device linear probes at `10%` and `100%` label fractions.
...
- `Supervised baseline`: rerun and standardize a full-label fused supervised baseline as the canonical comparator, then add fused label-fraction baselines at `10%` and `100%` using the exact same subset-selection logic used for probe training.
...
- `Comparison outputs`: generate one summary table covering `fusion_full`, `fusion_10pct`, `contrastive_pair_probe_10pct`, `contrastive_pair_probe_100pct`, `contrastive_phone_probe_10pct`, `contrastive_phone_probe_100pct`, `contrastive_watch_probe_10pct`, and `contrastive_watch_probe_100pct`.
...
- `Acceptance criteria`: the phase is complete when the repo can reproducibly produce a canonical fused supervised baseline, one contrastive checkpoint, and all paired/phone/watch probes at `10%` and `100%`, with metrics and artifacts saved in a standardized layout.
...
- The canonical supervised comparator is a full-label fused model, with matched `10%` fusion training added for fair label-efficiency comparison.
